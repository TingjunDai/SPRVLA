...
from olmo.models.sprvla.collator import MMCollator
from olmo.models.sprvla.data_formatter import DataFormatter
from olmo.models.sprvla.model_preprocessor import Preprocessor, SPRVLAPreprocessorConfig
...
from olmo.nn.vision_backbone import SPRVLAVisionBackbone, SPRVLAVisionBackboneConfig
...
class SPRVLAConfig(BaseModelConfig):
    """SPRVLA model configuration"""
    # _model_name: ClassVar[str]

    _model_name: ClassVar[str] = "sprvla"

    llm: LlmConfig = field(default_factory=LlmConfig)
    """LLM to use for generation"""

    vision_backbone: Optional[SPRVLAVisionBackboneConfig] = field(default_factory=SPRVLAVisionBackboneConfig)
    ...
    mm_preprocessor: SPRVLAPreprocessorConfig = field(default_factory=SPRVLAPreprocessorConfig)
    ...
    def update_legacy_settings(cls, config: D) -> D:
        ...
        if config.vision_backbone is not None:
            config.vision_backbone = SPRVLAVisionBackboneConfig.update_legacy_settings(config.vision_backbone)
        config.data_formatter = DataFormatter.update_legacy_settings(config.data_formatter)
        config.mm_preprocessor = SPRVLAPreprocessorConfig.update_legacy_settings(config.mm_preprocessor)
        return config
...
class SPRVLA(ModelBase):
    """SPRVLA model"""

    def __init__(self, config: SPRVLAConfig, device=None):
        super().__init__()
        self.config = config
    ...
    def get_fsdp_wrap_policy(self, wrap_strategy: Optional[FSDPWrapStrategy] = None):
        ...
        wrap_layer_names = (OLMoBlock, ResidualAttentionBlock, DinoResidualAttentionBlock, SPRVLAVisionBackbone, VisionTransformer, SiglipVisionTransformer, DinoVisionTransformer)
