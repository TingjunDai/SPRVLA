...
- Preserves class organization for LIBERO post-training datasets.
- get():
  * Multi-camera configs (e.g., 'image', 'wrist') -> return a list
...
class RobotHfDataset(HfDataset):
    ...
    - Subclasses set PATH (Hub repo like 'allenai/SPRVLA-Pretraining-Mixture').
    ...
    def __init__(
        ...
        **hf_kwargs,
        ...
        local_dataset = self._try_load_local(dataset_name, split)
        if local_dataset is not None:
            # Use local dataset
            print(f"[RobotHfDataset] Loading from local path for {dataset_name}/{split}")
            self.dataset = local_dataset
            self.keep_in_memory = keep_in_memory
        else:
            print(f"[RobotHfDataset] Loading from HuggingFace for {dataset_name}/{split}")
    ...
    def _try_load_local(self, dataset_name: str, split: str):
        """Try to load dataset from local directory."""
        local_data_dir = os.environ.get('SPRVLA_DATA_DIR')
        if not local_data_dir:
            return None

        local_path = os.path.join(local_data_dir, dataset_name, split)

        if not os.path.exists(local_path):
            return None

        # Find parquet files
        parquet_files = sorted(Path(local_path).glob("*.parquet"))
        if not parquet_files:
            print(f"[Warning] Local path exists but no parquet files found: {local_path}")
            return None

        print(f"[RobotHfDataset] Found {len(parquet_files)} local parquet files in {local_path}")
        ...
        try:
            ...
            print(f"[Warning] Failed to load local dataset: {e}")
            return None
