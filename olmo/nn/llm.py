...
class Llm(nn.Module):
    ...
    def reset_with_pretrained_weights(self) -> None:
        if self.config.init_path is None:
            ...
            if is_sharded:
                state_dict_path = rank0_resource_path(device, parent, name, cache_dir=os.environ.get("SPRVLA_CACHE_DIR"))
            else:
                state_dict_path = resource_path(parent, name, cache_dir=os.environ.get("SPRVLA_CACHE_DIR"))
    ...
    # No forward method since this is only used as part of a `SPRVLA` model
