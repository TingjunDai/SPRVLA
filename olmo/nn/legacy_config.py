...
def convert_vision_backbone(config) -> SPRVLAVisionBackboneConfig:
...
def convert_legacy_config(config):
    ...
    preprocessor = {k: MISSING for k in SPRVLAPreprocessorConfig().asdict()}
    data_formater = {k: MISSING for k in DataFormatter().asdict()}
    image_vit_keys = {k: MISSING for k in VitConfig().asdict()}
    vision_backbone = {k: MISSING for k in SPRVLAVisionBackboneConfig().asdict()}
    llm_args = {}
