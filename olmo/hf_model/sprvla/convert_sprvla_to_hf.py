...
from olmo.models.sprvla.sprvla import SPRVLA, SPRVLAConfig as ModelConfig
...
from modeling_sprvla import SPRVLAForCausalLM, SPRVLAForActionReasoning
from processing_sprvla import SPRVLAProcessor
from image_processing_sprvla import SPRVLAImageProcessor
...
def convert_config(model_config: ModelConfig) -> SPRVLAConfig:
    ...
    sprvla_vit_config = SPRVLAVitConfig(
    ...
    sprvla_adapter_config = SPRVLAAdapterConfig(
    ...
    sprvla_llm_config = SPRVLALlmConfig(
    ...
    sprvla_config = SPRVLAConfig(
        vit_config=sprvla_vit_config,
        adapter_config=sprvla_adapter_config,
        llm_config=sprvla_llm_config,
        use_cache=True,
...
def convert_sprvla(state_dict: dict[str, Any], config: Union[SPRVLALlmConfig, SPRVLAConfig], text_only: bool) -> dict[str, Any]:
    if text_only:
        base_model_prefix = SPRVLAForCausalLM.base_model_prefix
        ...
        base_model_prefix = SPRVLAForActionReasoning.base_model_prefix
        new_state_dict = convert_lm_head_and_prefix(state_dict, base_model_prefix)
        model_prefix = f"{base_model_prefix}.transformer"
    qkv_bias = config.qkv_bias if isinstance(config, SPRVLALlmConfig) else config.llm_config.qkv_bias
    use_qk_norm = config.use_qk_norm if isinstance(config, SPRVLALlmConfig) else config.llm_config.use_qk_norm
...
def convert_text_only_model(
    ...
    hf_config: SPRVLALlmConfig,
    precision: str,
) -> SPRVLAForCausalLM:
    """Convert text only model to HF-compatible model"""
    with torch.device("meta"):
        model: SPRVLA = model_config.build_model()
        hf_model = SPRVLAForCausalLM(hf_config)
    ...
    new_state_dict = convert_sprvla(state_dict, hf_config, text_only=True)
    hf_model.eval()
    if precision == "bf16":
...
def convert_model(
    checkpoint_dir: str,
    model_config: ModelConfig,
    hf_config: SPRVLAConfig,
    precision: str,
) -> SPRVLAForActionReasoning:
    """Convert model to HF-compatible model"""
    with torch.device("meta"):
        model: SPRVLA = model_config.build_model()
        hf_model = SPRVLAForActionReasoning(hf_config)
    model.to_empty(device=torch.device("cpu"))
    hf_model.to_empty(device=torch.device("cpu"))
    ...
    new_state_dict = convert_sprvla(state_dict, hf_config, text_only=False)
    hf_model.eval()
    if precision == "bf16":
        hf_model = hf_model.to(torch.bfloat16)
    elif precision == "fp32":
...
def save(
    ...
    model_file = os.path.join(output_dir, "modeling_sprvla.py")
    if not os.path.exists(model_file):
        logger.warning(f"Copy model file to {model_file} manually")
        shutil.copyfile(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "modeling_sprvla.py"),
            model_file,
        )
    ...
    if text_only and "AutoModelForCausalLM" not in auto_map:
        ...
        auto_map["AutoModelForCausalLM"] = "modeling_sprvla.SPRVLAForCausalLM"
        ...
        auto_map["AutoModelForImageTextToText"] = "modeling_sprvla.SPRVLAForActionReasoning"
        with open(os.path.join(output_dir, "config.json"), "w") as f:
            json.dump(config, f, indent=2)
    ...
    if text_only:
        ...
        processor = SPRVLAProcessor(
            SPRVLAImageProcessor(
    ...
    if isinstance(tokenizer, Qwen2TokenizerFast) and override_tokenizer:
        ...
        if not os.path.exists(custom_tokenizer_file):
            ...
            shutil.copyfile(
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "custom_tokenizer.py"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert SPRVLA checkpoint to HuggingFace format."
    )
    parser.add_argument("--checkpoint_dir", help="Location of SPRVLA checkpoint.")
