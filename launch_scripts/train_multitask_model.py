...
from utils import get_evaluation, DEBUG_MODEL
...
if __name__ == "__main__":
    ...
    eval_tasks = []
    eval_tasks_loss = []
    if args.mixture in ["libero-spatial"]:
    ...
    elif args.mixture in ["franka_real"]:
        # this will be uniform sampling
        tasks = [
            ["franka_real", [
                "franka_real",
    ...
    if debug:
        ...
        if is_hf_remote:
            ...
            model_cfg = SPRVLAConfig.load(p)
        if exists(join(checkpoint, "model.yaml")):
            model_cfg = SPRVLAConfig.load(join(checkpoint, "model.yaml"))
        elif exists(join(checkpoint, "config.yaml")):
            model_cfg = SPRVLAConfig.load(join(checkpoint, "config.yaml"), key="model")
